...
static int
gen_vals(int test_vals[], int min, int max)
{
  int i;
  int test_count = 0;
  ...
  /* Otherwise, need to sample.
     Do so near the boundaries and for a
     few random cases */
...
static int
test_0_arg(funct_t f, funct_t ft, char *name, int report)
{
  int r = f();
  int rt = ft();
  int error =  (r != rt);
...
static int
test_1_arg(funct_t f, funct_t ft, int arg1, char *name, int report)
{
...
test_2_arg(funct_t f, funct_t ft, int arg1, int arg2, char *name, int report)
{
  ...
  if (error && report)
    printf("Test %s(%d[0x%x],%d[0x%x]) failed.\n  Gives %d[0x%x]."
           "  Should be %d[0x%x]\n",
...
static int
test_3_arg(funct_t f, funct_t ft, int arg1, int arg2, int arg3,
           char *name, int report)
{
  ...
  if (error && report)
    printf("Test %s(%d[0x%x],%d[0x%x],%d[0x%x]) failed.\n  Gives %d[0x%x]."
           "  Should be %d[0x%x]\n",
...
static int
test_function(test_ptr t, int report) {
